...
import { Bell, Search, Clock, CheckCircle, Calendar, TrendingUp, ChevronRight, Star, Trophy, Loader2, Users, Send, Target, BookOpen, GraduationCap, ArrowRight, School, MessageSquare } from 'lucide-react';
...
export function StudentDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const scoreHistory = results.slice().reverse().map(r => ({
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        ...
        <p className="text-slate-500 font-bold animate-pulse uppercase tracking-widest text-xs">Đang tải dữ liệu tổng quan...</p>
      </div>
    );
  }
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-16">
          <div className="max-w-2xl space-y-8">
            ...
            <p className="text-brand-50/90 text-xl font-medium leading-relaxed max-w-xl">
              Chào <span className="text-white font-black underline decoration-brand-200 decoration-2 underline-offset-8">{user?.name}</span>! Hiện có <span className="text-white font-black underline decoration-amber-400 decoration-2 underline-offset-8">{stats.totalUpcoming} kỳ thi</span> đang chờ bạn khám phá. Hãy bắt đầu ngay thôi!
            </p>
            <div className="flex flex-wrap gap-6 pt-4">
              <Link to="/student/exams" className="px-10 py-5 bg-white text-brand-700 rounded-3xl font-black hover:bg-amber-400 hover:text-gray-900 transition-all transform hover:scale-105 active:scale-95 shadow-2xl shadow-brand-900/20 flex items-center gap-3 group/btn">
                Bắt đầu học ngay 
              ...
              <button onClick={() => document.getElementById('join-class-input')?.focus()} className="px-10 py-5 bg-white/10 backdrop-blur-xl border border-white/20 text-white rounded-3xl font-black hover:bg-white/20 transition-all flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8">
          ...
          <div className="relative z-10">
            <div className="flex items-center justify-between mb-10">
              ...
              <div className="text-right">
                <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none mb-2">Tiến độ tổng thể</p>
                <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-50 dark:bg-emerald-500/10 rounded-xl border border-emerald-100 dark:border-emerald-500/20">
                  ...
                  <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">Đang tăng trưởng ✨</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 mb-10">
              <div>
                <h3 className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] mb-2">Bài thi hoàn thành</h3>
                <div className="flex items-baseline gap-3">
                  <span className="text-7xl font-black text-slate-900 dark:text-white tracking-tighter leading-none">{stats.completedExamsCount}</span>
                  <span className="text-slate-300 dark:text-slate-700 font-black text-3xl">/ 20</span>
                </div>
              </div>
              <div className="flex-1 h-[120px] pb-2 relative min-w-[200px]">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={scoreHistory}>
                    <Area type="monotone" dataKey="score" stroke="#1a56db" strokeWidth={5} fill="#1a56db" fillOpacity={0.05} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between items-center">
                <p className="text-[11px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest leading-none">Cấp độ 4</p>
                <p className="text-[11px] font-black text-brand uppercase tracking-widest leading-none">Hoàn thành {Math.round((stats.completedExamsCount / 20) * 100)}% mục tiêu</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/10 dark:shadow-none hover:shadow-2xl hover:shadow-brand/5 transition-all duration-500 group flex flex-col justify-between overflow-hidden relative">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Điểm trung bình</p>
            <div className="flex items-baseline gap-2">
              <h3 className="text-6xl font-black text-slate-900 dark:text-white tracking-tighter">{stats.averageScore}</h3>
        ...
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/10 dark:shadow-none hover:shadow-2xl hover:shadow-brand/5 transition-all duration-500 group flex flex-col justify-between overflow-hidden relative">
          ...
          <div>
            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Thời gian ôn luyện</p>
            <div className="flex items-baseline gap-2">
              <h3 className="text-6xl font-black text-slate-900 dark:text-white tracking-tighter">{stats.studyHours}</h3>
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Lịch thi sắp tới</h2>
            </div>
            <Link to="/student/exams" className="group flex items-center gap-2 text-sm font-black text-brand">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {upcomingExams.map((exam) => (
              <div key={exam._id} className="bg-white dark:bg-slate-900 p-8 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                <div className="flex flex-col h-full gap-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      <span className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded-xl">
                        <Calendar className="w-3.5 h-3.5" /> {new Date(exam.endAt).toLocaleDateString('vi-VN')}
                      </span>
                    </div>
                  </div>
            ...
            {upcomingExams.length === 0 && (
                <div className="md:col-span-2 text-center py-20 bg-slate-50/50 dark:bg-slate-900/50 rounded-[40px] border-2 border-dashed border-slate-200 dark:border-slate-800">
                    ...
                    <h3 className="text-xl font-black text-slate-900 dark:text-white">Tất cả đã hoàn thành!</h3>
                    <p className="text-slate-400 dark:text-slate-500 mt-2 font-bold uppercase tracking-widest text-[10px]">Bạn không có lịch thi nào trong hôm nay</p>
                </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-10">
           ...
           <div className="bg-white dark:bg-slate-900 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden">
             <div className="p-8 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between">
               ...
               <Link to="/student/results" className="text-[10px] font-black text-brand uppercase tracking-widest hover:underline">Tất cả</Link>
             </div>
             <div className="divide-y divide-slate-50 dark:divide-slate-800/50">
               {recentResults.map((result) => (
                 <div key={result.id} className="p-8 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
                   <div className="flex justify-between items-center mb-4">
                     <p className="font-black text-slate-800 dark:text-slate-200 text-sm line-clamp-1 group-hover:text-brand transition-colors">{result.title}</p>
                     <div className={cn(
                   ...
                   <div className="flex items-center gap-4">
                      <span className="text-[9px] font-black uppercase tracking-widest text-brand bg-brand/5 dark:bg-brand/10 px-2 py-1 rounded-md border border-brand/10">{result.subject}</span>
                      <span className="text-[9px] font-black uppercase tracking-widest text-slate-300 dark:text-slate-600">{new Date(result.submittedAt).toLocaleDateString('vi-VN')}</span>
                   </div>
                 </div>
               ))}
             </div>
        ...
        <div className="space-y-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Lớp học của bạn</h2>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {classrooms.map((cls) => (
              <div key={cls._id} className="bg-white dark:bg-slate-900 p-8 rounded-[32px] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group">
                <div className="flex flex-col h-full gap-8">
                  <div className="flex items-start justify-between">
                    <div className="w-16 h-16 bg-blue-50 dark:bg-blue-500/10 rounded-[24px] flex items-center justify-center text-blue-600 dark:text-blue-400 font-black text-2xl shadow-inner group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
                      {(cls.subject || 'CL').substring(0, 2).toUpperCase()}
                    </div>
                  </div>
                  
                  <div className="space-y-3">
                    <h3 className="text-xl font-black text-slate-900 dark:text-white line-clamp-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-tight">
                      {cls.name}
                    </h3>
                    <div className="flex flex-wrap gap-3">
                      <span className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded-xl">
                        <Users className="w-3.5 h-3.5" /> {cls.grade}
                      </span>
                      <span className="inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded-xl">
                        {cls.subject}
                      </span>
                  ...
                  <Link 
                    to={`/student/classrooms/${cls._id}/chat`}
                    className="w-full py-4 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-2xl font-black text-sm hover:from-purple-600 hover:to-purple-700 transition-all transform active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-purple-500/20"
                  >

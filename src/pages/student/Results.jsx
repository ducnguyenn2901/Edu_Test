...
import { Trophy, TrendingUp, Clock, Calendar, CheckCircle, ChevronRight, BarChart2, AlertCircle, FileText, Target, Award, BookOpen, Activity, ArrowRight, Star } from 'lucide-react';
...
export function StudentResults() {
  ...
  const totalTimeSeconds = results.reduce(
    (acc, curr) => acc + (curr.timeSpentSeconds || 0),
    0,
  );
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div className="space-y-1">
          ...
          <h1 className="text-4xl font-black text-gray-900 dark:text-white tracking-tight">Kết quả học tập</h1>
          <p className="text-gray-500 dark:text-slate-400 font-medium">Phân tích sâu và theo dõi sự tiến bộ của bạn qua từng kỳ thi.</p>
        </div>
        ...
        <div className="relative overflow-hidden rounded-4xl bg-gradient-brand p-8 md:p-10 text-white shadow-2xl shadow-brand/20">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex items-center gap-6">
              ...
              <div>
                <p className="text-brand-light text-[10px] font-black uppercase tracking-widest">Vừa hoàn thành</p>
            ...
            <div className="flex items-center gap-8 bg-white/10 backdrop-blur-md px-8 py-6 rounded-3xl border border-white/20">
               <div className="text-center">
                  <p className="text-[10px] font-black text-brand-light uppercase tracking-widest mb-1">Điểm số</p>
                  <p className="text-4xl font-black">{latestResult.score}</p>
               </div>
               <div className="w-px h-10 bg-white/20"></div>
               <div className="text-center">
                  <p className="text-[10px] font-black text-brand-light uppercase tracking-widest mb-1">Độ chính xác</p>
                  <p className="text-4xl font-black">{Math.round((latestResult.correctAnswers / latestResult.totalQuestions) * 100)}%</p>
               </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white dark:bg-slate-900 p-8 rounded-4xl border border-gray-100 dark:border-slate-800 shadow-xl shadow-gray-200/40 dark:shadow-none group relative overflow-hidden">
               ...
               <div className="relative z-10">
                 ...
                 <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-1">Điểm trung bình</p>
                 <div className="flex items-baseline gap-2">
                   <h3 className="text-4xl font-black text-gray-900 dark:text-white">{averageScore}</h3>
                   <span className="text-xs font-bold text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded-lg border border-emerald-100">↗ +0.2</span>
                 </div>
               </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-8 rounded-4xl border border-gray-100 dark:border-slate-800 shadow-xl shadow-gray-200/40 dark:shadow-none group relative overflow-hidden">
               ...
               <div className="relative z-10">
                 ...
                 <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-1">Bài thi hoàn thành</p>
                 <h3 className="text-4xl font-black text-gray-900 dark:text-white">{results.length}</h3>
               </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-8 rounded-4xl border border-gray-100 dark:border-slate-800 shadow-xl shadow-gray-200/40 dark:shadow-none group relative overflow-hidden">
               ...
               <div className="relative z-10">
                 ...
                 <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-1">Thời gian ôn tập</p>
                 <h3 className="text-4xl font-black text-gray-900 dark:text-white">{formatHours(totalTimeSeconds)}</h3>
               </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-4xl border border-gray-100 dark:border-slate-800 shadow-xl shadow-gray-200/30 dark:shadow-none overflow-hidden">
            <div className="px-8 py-6 border-b border-gray-50 dark:border-slate-800 flex items-center justify-between">
               ...
               <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{results.length} bản ghi</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/50 dark:bg-slate-800/30">
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-400">Tên bài thi & Môn học</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-400 text-center">Kết quả</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-400">Thời gian làm</th>
                    <th className="px-8 py-5 text-[10px] font-black uppercase tracking-widest text-gray-400 text-right">Ngày nộp</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50 dark:divide-slate-800">
                  {sortedByTime.map((res) => (
                    <tr key={res.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors group cursor-pointer" onClick={() => navigate(`/student/results/${res.examId}/${res.attemptId}`)}>
                      <td className="px-8 py-6">
                         <div className="flex items-center gap-4">
                            ...
                            <div>
                               <p className="font-black text-gray-900 dark:text-white group-hover:text-brand transition-colors line-clamp-1">{res.title}</p>
                               <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-0.5">{res.subject}</p>
                            </div>
                         </div>
                      </td>
                      <td className="px-8 py-6">
                         <div className="flex flex-col items-center gap-1">
                            <span className={cn(
                               ...
                               res.score >= 8 ? "bg-emerald-50 text-emerald-600 border-emerald-100" :
                               res.score >= 5 ? "bg-amber-50 text-amber-600 border-amber-100" :
                               "bg-rose-50 text-rose-600 border-rose-100"
                            )}>{res.score}</span>
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{res.correctAnswers}/{res.totalQuestions} đúng</span>
                         </div>
                      </td>
                      <td className="px-8 py-6">
                         <div className="flex items-center gap-2 text-sm font-bold text-gray-500">
                            <Clock className="w-4 h-4 text-brand" /> {formatTime(res.timeSpentSeconds)}
                         </div>
                      </td>
                      <td className="px-8 py-6 text-right">
                         <div className="flex flex-col items-end gap-1">
                            <p className="text-sm font-bold text-gray-900 dark:text-white">{new Date(res.submittedAt).toLocaleDateString('vi-VN')}</p>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{new Date(res.submittedAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</p>
                         </div>
                      </td>
                    </tr>
                  ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                 ...
                 {subjectList.map((item, idx) => (
                   <div key={idx} className="space-y-3">
                      <div className="flex justify-between items-end">
                         <div>
                            <p className="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tight">{item.subject}</p>
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{item.count} bài thi</p>
                         </div>
           ...
           <div className="bg-white dark:bg-slate-900 p-8 rounded-4xl border border-gray-100 dark:border-slate-800 shadow-xl shadow-gray-200/40 dark:shadow-none">
              ...
              <div className="space-y-4">
                 {examSummaryList.sort((a, b) => b.best.score - a.best.score).slice(0, 5).map((summary, idx) => (
                   <div key={idx} className="flex items-center gap-4 p-5 bg-gray-50/50 dark:bg-slate-800/50 rounded-3xl border border-gray-100 dark:border-slate-800 group hover:bg-brand/5 transition-all">
                      ...
                      <div className="flex-1 min-w-0">
                         <p className="text-sm font-black text-gray-900 dark:text-white truncate group-hover:text-brand transition-colors">{summary.title}</p>
                         <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">{summary.best.subject} • {summary.best.correctAnswers}/{summary.best.totalQuestions} câu</p>
                      </div>
                      <div className="text-right">
                         <p className="text-xl font-black text-brand">{summary.best.score}</p>
                         <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Điểm</p>
                      </div>
                   </div>
                 ))}
                 {examSummaryList.length === 0 && (
                   <p className="text-center text-gray-400 font-bold py-12">Chưa có dữ liệu thành tích</p>
                 )}
              </div>
           </div>
        </div>

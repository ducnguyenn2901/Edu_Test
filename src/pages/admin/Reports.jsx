    ...
    const fetchReports = async () => {
      try {
        ...
        const mappedData = {
          ...
          subjectDistribution: Object.entries(backendData.attempts.bySubject || {}).map(
            ([name, stat]) => ({
              name,
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        ...
        <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px]">
          Đang tổng hợp báo cáo hệ thống...
        </p>
      </div>
    );
  }
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">
            Phân tích toàn diện hiệu suất và sự tăng trưởng của hệ thống EduTest.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {metricCards.map((metric, i) => (
          ...
          >
            <div
              className={cn(
                'w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 shadow-sm',
                metric.bg,
                metric.color,
              )}
            >
          ...
          <div className="flex items-center justify-between mb-12 relative z-10">
            <div>
              ...
              <p className="text-sm font-medium text-slate-400 mt-1">
                Số lượng tài khoản đăng ký theo thời gian
              </p>
            </div>
            ...
            {subjectDistribution?.slice(0, 4).map((item, i) => (
              <div key={i} className="flex items-center justify-between group/item cursor-default">
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-sm font-bold text-slate-600 dark:text-slate-300 group-hover/item:text-brand transition-colors">
                    {item.name}
                  </span>
                </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-slate-400 mt-1">
                Thống kê số lượt làm bài theo ngày
              </p>
            </div>
          </div>

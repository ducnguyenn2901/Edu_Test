...
export function TeacherDashboard() {
  ...
  const { stats, recentQuestions, monthlyStats } = data || {
    stats: {},
    recentQuestions: [],
    monthlyStats: [],
  };
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium text-lg">
            Chào mừng bạn quay trở lại. Hãy xem các hoạt động mới nhất hôm nay.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5">
        {statCards.map((stat, idx) => (
          <div
            key={idx}
            className="group relative bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/10 dark:shadow-none hover:shadow-2xl hover:shadow-brand/5 dark:hover:border-brand/20 transition-all duration-500 hover:-translate-y-1.5 overflow-hidden"
          >
            <div
              className={`absolute top-0 right-0 w-32 h-32 -mr-12 -mt-12 bg-gradient-to-br ${stat.gradient} opacity-[0.03] group-hover:opacity-[0.08] rounded-full transition-all duration-700 group-hover:scale-150`}
            />

            <div
              className={`w-14 h-14 bg-gradient-to-br ${stat.gradient} rounded-2xl flex items-center justify-center text-white mb-6 shadow-lg ${stat.shadow} group-hover:scale-110 group-hover:rotate-3 transition-all duration-500`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-4 relative z-10">
            <div>
              ...
              <p className="text-sm font-medium text-slate-400 dark:text-slate-500 mt-1">
                Thống kê dữ liệu trong 6 tháng vừa qua
              </p>
            </div>
        ...
        <div className="bg-gradient-brand rounded-[2.5rem] p-10 text-white flex flex-col justify-between shadow-2xl shadow-brand/20 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <p className="text-brand-50/80 mt-3 font-medium text-sm leading-relaxed">
              Truy cập nhanh các chức năng quan trọng nhất để tối ưu thời gian giảng dạy.
            </p>
          </div>
          ...
          <div className="space-y-4 mt-10 relative z-10">
            <Link
              ...
              className="flex items-center justify-between p-5 bg-white/10 hover:bg-white/20 rounded-2xl transition-all group/btn backdrop-blur-xl border border-white/10 hover:border-white/30"
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-brand-50/60 uppercase tracking-widest font-black mt-0.5">
                    Mới
                  </p>
                </div>
              </div>
          ...
          <div className="px-10 py-8 border-b border-gray-50 dark:border-slate-800 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs font-medium text-gray-400 dark:text-slate-500 mt-1">
                Các câu hỏi bạn vừa chỉnh sửa hoặc tạo mới
              </p>
            </div>
            <Link
              to="/teacher/questions"
              className="flex items-center gap-2 text-xs font-black text-brand uppercase tracking-widest hover:bg-brand/5 px-5 py-3 rounded-2xl transition-all border border-transparent hover:border-brand/10"
            >
              Xem tất cả
          ...
          <div className="divide-y divide-gray-50 dark:divide-slate-800/50">
            {recentQuestions.length > 0 ? (
              recentQuestions.map((q) => (
                <div
                  key={q._id}
                  className="px-10 py-6 flex items-start gap-6 hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-all group/item"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-black text-gray-800 dark:text-slate-200 line-clamp-1 leading-snug group-hover/item:text-brand transition-colors">
                      {q.content}
                    </p>
                    <div className="flex flex-wrap items-center gap-2 mt-3">
                      <span className="text-[9px] font-black uppercase tracking-widest bg-brand/5 dark:bg-brand/10 text-brand px-3 py-1.5 rounded-lg border border-brand/10">
                        {q.subject}
                      </span>
                      <span className="text-[9px] font-black uppercase tracking-widest bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 px-3 py-1.5 rounded-lg">
                        {q.grade}
                      </span>
                      <span
                        className={`text-[9px] font-black uppercase tracking-widest px-3 py-1.5 rounded-lg border ${
                          q.difficulty === 'Dễ'
              ...
              <div className="p-12 text-center">
                ...
                <p className="text-gray-400 dark:text-slate-500 font-bold uppercase tracking-widest text-xs">
                  Chưa có câu hỏi nào
                </p>
                <Link
                  to="/teacher/questions/new"
                  className="inline-block mt-4 text-brand font-black text-xs uppercase tracking-widest hover:underline underline-offset-8"
                >
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-gray-100 dark:border-slate-800 shadow-xl shadow-gray-200/10 dark:shadow-none p-8 relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <p className="text-sm text-gray-500 dark:text-slate-400 leading-relaxed font-medium">
                Bạn có thể nhập câu hỏi từ tệp Excel hoặc Word để tiết kiệm thời gian hơn so với
                việc nhập thủ công từng câu.
              </p>
              <button className="mt-6 text-xs font-black text-amber-600 dark:text-amber-500 uppercase tracking-widest flex items-center gap-2 group/tip">
                Tìm hiểu thêm{' '}
                <ArrowRight className="w-4 h-4 group-hover/tip:translate-x-1 transition-transform" />
              </button>
            </div>
          </div>
          ...
          <div className="bg-slate-900 dark:bg-slate-800 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-400 text-sm leading-relaxed font-medium mb-6">
                EduTest Pro cho phép bạn tạo mã QR để học sinh tham gia lớp học một cách nhanh chóng
                và an toàn.
              </p>
              <Link
                to="/teacher/classrooms"
                className="inline-flex items-center gap-3 px-6 py-3 bg-brand text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-brand/20 hover:scale-105 transition-all"
              >
                Đến lớp học
                <ArrowRight className="w-4 h-4" />
              </Link>

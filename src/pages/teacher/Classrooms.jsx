...
import { Loader2, UserPlus, QrCode, Check, X, PlusCircle, School, BookOpen, Users, ChevronRight, MoreVertical, Edit3, Trash2, Upload, MessageSquare } from 'lucide-react';
...
export function Classrooms() {
  ...
  const fetchClassrooms = async () => {
    try {
      setLoading(true);
      const response = await classroomApi.getAll({ teacherId: user?._id || user?.id });
  ...
  const handleApprove = async (classroomId, studentId, action) => {
    try {
      ...
      showToast({ type: 'success', title: 'Thành công', message: `Đã ${action === 'approve' ? 'duyệt' : 'từ chối'} học sinh.` });
  ...
  if (loading && classrooms.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
        ...
        <p className="text-gray-500 font-medium animate-pulse uppercase tracking-widest text-xs">Đang tải danh sách lớp học...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight uppercase">Quản lý Lớp học</h1>
          <p className="text-gray-500 mt-1 font-medium italic">Tổ chức lớp học, mời học sinh và quản lý yêu cầu tham gia.</p>
        </div>
        <button 
          onClick={() => {
            setIsEditing(false);
      ...
      {classrooms.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {classrooms.map(cls => (
            <div key={cls._id} className="group bg-white rounded-[40px] border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden flex flex-col relative">
              ...
              <div className="p-10 flex-1">
                ...
                <h3 className="font-black text-3xl text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-1 mb-3 tracking-tight">{cls.name}</h3>
                <div className="flex items-center gap-3 text-[10px] font-black text-gray-400 uppercase tracking-[0.15em]">
                  ...
                  <span className="bg-indigo-50 text-indigo-600 px-3 py-1 rounded-lg">{cls.subject}</span>
                </div>
                ...
                <div className="mt-12 flex items-center gap-10">
                  ...
                  >
                     <span className="text-3xl font-black text-gray-900 dark:text-white">{cls.students?.length || 0}</span>
                     <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1 flex items-center gap-1">
                        Học sinh {showStudentsId === cls._id ? <X className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                     </span>
                  </button>
                  <div className="w-px h-12 bg-gray-100 dark:bg-slate-800" />
                  <div className="flex flex-col">
                     <span className="text-3xl font-black text-amber-500">{cls.pendingStudents?.length || 0}</span>
                     <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">Chờ duyệt</span>
                  </div>
                </div>
              </div>
                ...
                <div className="px-8 pb-6 animate-in slide-in-from-top-4 duration-300">
                   <div className="bg-slate-50 dark:bg-slate-800/50 rounded-3xl p-6 border border-slate-100 dark:border-slate-800">
                      <div className="flex items-center justify-between mb-4">
                         <h4 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Danh sách học sinh</h4>
                         <span className="text-[10px] font-bold text-slate-400">{cls.students?.length || 0} thành viên</span>
                      </div>
                      <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                         {cls.students && cls.students.length > 0 ? (
                            cls.students.map(student => (
                               <div key={student._id} className="flex items-center gap-3 p-3 bg-white dark:bg-slate-800 rounded-2xl border border-slate-50 dark:border-slate-700 shadow-sm group">
                                  ...
                                  <div className="min-w-0 flex-1">
                                     ...
                                     <p className="text-[9px] text-slate-400 truncate tracking-tight">{student.email}</p>
                                  </div>
                                  <button 
                            ...
                            <p className="text-center py-4 text-xs text-slate-400 italic">Lớp chưa có học sinh</p>
                         )}
                      </div>
                   </div>
                </div>
                ...
                <div className="px-8 pb-6">
                  <div className="bg-amber-50 rounded-[32px] p-8 border border-amber-100 shadow-inner">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <span className="bg-amber-500 text-white text-[10px] font-black px-2.5 py-1 rounded-full">{cls.pendingStudents.length}</span>
                    </div>
                    <div className="space-y-4 max-h-56 overflow-y-auto pr-2 custom-scrollbar">
                      {cls.pendingStudents.map(student => (
                        <div key={student._id} className="flex items-center justify-between bg-white/80 backdrop-blur-sm p-4 rounded-2xl border border-white shadow-sm hover:shadow-md transition-all group/req">
                          <div className="min-w-0">
                            <p className="font-black text-sm text-gray-800 truncate tracking-tight">{student.name}</p>
                            <p className="text-[9px] text-gray-400 font-bold truncate uppercase tracking-widest mt-0.5">{student.email}</p>
                          </div>
                          <div className="flex gap-2 ml-4">
                            <button onClick={() => handleApprove(cls._id, student._id, 'reject')} className="p-2.5 text-rose-500 hover:bg-rose-50 rounded-xl transition-all active:scale-90" title="Từ chối">
                              <X className="w-5 h-5" />
                            </button>
                            <button onClick={() => handleApprove(cls._id, student._id, 'approve')} className="p-2.5 text-emerald-500 hover:bg-emerald-50 rounded-xl transition-all active:scale-90" title="Chấp nhận">
              ...
              <div className="p-8 pt-0 mt-auto flex gap-3">
                <Link
                  to={`/teacher/classrooms/${cls._id}/chat`}
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-4 text-[10px] font-black uppercase tracking-widest text-purple-600 bg-purple-50 rounded-2xl hover:bg-purple-600 hover:text-white transition-all duration-300 group/btn shadow-sm"
                >

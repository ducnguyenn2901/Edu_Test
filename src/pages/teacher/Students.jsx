...
export function Students() {
  ...
  const fetchStudents = async () => {
    try {
      ...
      const message =
        _err?.response?.data?.message || 'Không thể tải danh sách học sinh. Vui lòng thử lại sau.';
      showToast({
        type: 'error',
        title: 'Lỗi',
        message: message,
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="mt-2 text-blue-100 max-w-xl font-medium">
              Quản lý danh sách học sinh, phê duyệt yêu cầu tham gia và theo dõi tiến độ học tập cá
              nhân.
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-4xl border border-slate-100 dark:border-slate-800 shadow-premium hover:shadow-premium-hover transition-all group overflow-hidden relative">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {gradeCounts.map((g) => (
              <span
                key={g.grade}
                className="px-3 py-1 bg-accent-50 dark:bg-accent-900/20 text-accent-700 dark:text-accent-400 rounded-xl text-[10px] font-black border border-accent-100 dark:border-accent-800 uppercase tracking-tight"
              >
                {g.grade}: {g.count}
              </span>
            ))}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-4xl border border-slate-100 dark:border-slate-800 shadow-premium overflow-hidden">
        ...
        <div className="flex items-center px-8 bg-slate-50/50 dark:bg-slate-800/20 border-b border-slate-100 dark:border-slate-800">
          ...
          >
            Học sinh chính thức
            {activeTab === 'active' && (
              <div className="absolute bottom-0 left-8 right-8 h-1 bg-brand-500 rounded-t-full shadow-lg shadow-brand-500/40" />
            )}
          </button>
          <button
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                {pendingRequests.length === 0 ? (
                  ...
                  pendingRequests.map((req) => (
                    <tr
                      key={`${req.classroomId}-${req._id || req.id}`}
                      className="hover:bg-warning-50/30 dark:hover:bg-warning-900/10 transition-all group"
                    >
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-black text-slate-900 dark:text-white group-hover:text-warning-600 transition-colors">
                              {req.name || 'Học sinh'}
                            </p>
                      ...
                      <td className="px-8 py-5 text-right">
                        <div className="flex items-center justify-end gap-3 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-x-2 group-hover:translate-x-0">
                          <button
                            onClick={() =>
                              handleApprove(req.classroomId, req._id || req.id, 'reject')
                            }
                          ...
                          <button
                            onClick={() =>
                              handleApprove(req.classroomId, req._id || req.id, 'approve')
                            }

...
export function ExamResults() {
  ...
  useEffect(() => {
    const load = async () => {
      if (!id) {
        setLoading(false);
        setError('Vui lòng vào mục "Kỳ thi" và chọn "Xem thống kê" cho một đề cụ thể.');
        return;
      }
      ...
      try {
        ...
        const message =
          e?.response?.data?.message || 'Không thể tải thống kê kỳ thi. Vui lòng thử lại sau.';
        setError(message);
      } finally {
        setLoading(false);
      }
  ...
  const easyQuestions = stats.perQuestion.filter((q) => q.difficultyFlag === 'too_easy');
  const hardQuestions = stats.perQuestion.filter((q) => q.difficultyFlag === 'too_hard');
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3 text-blue-100 font-medium text-sm">
              <span className="px-2.5 py-0.5 rounded-lg bg-white/10 border border-white/10">
                {exam.subject}
              </span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          ...
          },
          {
            ...
            bg: 'bg-accent-50',
            sub: `${stats.passCount} đạt • ${stats.failCount} trượt`,
          },
          {
            label: 'Số lượt nộp',
          ...
          <div
            key={i}
            className="bg-white dark:bg-slate-900 p-8 rounded-4xl border border-slate-100 dark:border-slate-800 shadow-premium hover:shadow-premium-hover transition-all group overflow-hidden relative"
          >
            <div
              className={cn(
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-slate-900 p-8 rounded-4xl border border-slate-100 dark:border-slate-800 shadow-premium">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            {stats.scoreDistribution.map((bucket, _i) => (
              <div
                key={bucket.range}
                className="relative p-6 rounded-3xl bg-slate-50/50 dark:bg-slate-800/30 border border-slate-100 dark:border-slate-800 group hover:bg-white dark:hover:bg-slate-800 transition-all hover:shadow-lg"
              >
            ...
            <div>
              ...
              <div className="space-y-3">
                {easyQuestions.length === 0 ? (
                  <p className="text-xs font-bold text-slate-400 italic bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl text-center">
                    Không có câu nào quá dễ
                  </p>
                ) : (
                  easyQuestions.map((q) => (
                    <div
                      key={q.questionId}
                      className="p-4 bg-emerald-50/50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/20 rounded-2xl group hover:shadow-md transition-all"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-black text-emerald-700 dark:text-emerald-500 uppercase tracking-widest">
                          Câu {q.index}
                        </span>
                        <span className="text-[10px] font-black text-emerald-600 bg-white dark:bg-slate-800 px-2 py-0.5 rounded-lg border border-emerald-100">
                          {Math.round((q.correctRate || 0) * 100)}% đúng
                        </span>
                      </div>
            ...
            <div className="pt-6 border-t border-slate-50 dark:border-slate-800">
              ...
              <div className="space-y-3">
                {hardQuestions.length === 0 ? (
                  <p className="text-xs font-bold text-slate-400 italic bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl text-center">
                    Không có câu nào quá khó
                  </p>
                ) : (
                  hardQuestions.map((q) => (
                    <div
                      key={q.questionId}
                      className="p-4 bg-rose-50/50 dark:bg-rose-900/10 border border-rose-100 dark:border-rose-900/20 rounded-2xl group hover:shadow-md transition-all"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-black text-rose-700 dark:text-rose-500 uppercase tracking-widest">
                          Câu {q.index}
                        </span>
                        <span className="text-[10px] font-black text-rose-600 bg-white dark:bg-slate-800 px-2 py-0.5 rounded-lg border border-rose-100">
                          {Math.round((q.correctRate || 0) * 100)}% đúng
                        </span>
                      </div>
                      <p className="text-xs font-bold text-slate-700 dark:text-slate-300 line-clamp-2 leading-relaxed">
              ...
              {stats.perQuestion.map((q) => (
                ...
                >
                  <td className="px-8 py-5">
                    <span className="text-xs font-black text-slate-900 dark:text-white bg-slate-100 dark:bg-slate-800 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700">
                      Câu {q.index}
                    </span>
                  </td>
                  <td className="px-8 py-5">
                    <p className="text-sm font-bold text-slate-700 dark:text-slate-300 line-clamp-1 group-hover:text-brand-600 transition-colors">
                      {q.content}
                    </p>
                  </td>
                  <td className="px-8 py-5">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-wider bg-slate-50 dark:bg-slate-800/50 px-3 py-1 rounded-full border border-slate-100 dark:border-slate-700">
                    ...
                    <span
                      className={cn(
                        ...
                        q.difficulty === 'Dễ' || q.difficulty === 'Nhận biết'
                          ...
                          : q.difficulty === 'Khó' || q.difficulty === 'Vận dụng'
                    ...
                    <div className="flex flex-col items-end gap-1.5">
                      <span
                        className={cn(
                          ...
                          q.difficultyFlag === 'too_easy'
                            ? 'text-emerald-500'
                            : q.difficultyFlag === 'too_hard'
                              ? 'text-rose-500'

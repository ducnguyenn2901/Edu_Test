...
import { Users, Folder, FileText, BarChart3, Settings, HelpCircle, LogOut, LayoutDashboard, Moon, Sun, User, ShieldCheck, ChevronRight } from 'lucide-react';
...
const sidebarItems = [
  { icon: Users, label: 'Quản lý người dùng', to: '/admin' },
  { icon: Folder, label: 'Danh mục & Khối lớp', to: '/admin/categories' },
  { icon: FileText, label: 'Quản lý kỳ thi', to: '/admin/exams' },
  { icon: BarChart3, label: 'Báo cáo hệ thống', to: '/admin/reports' },
  { icon: Users, label: 'Phân bổ lớp học', to: '/admin/classrooms' },
];

const systemItems = [
  { icon: User, label: 'Hồ sơ cá nhân', to: '/admin/profile' },
  { icon: Settings, label: 'Cấu hình hệ thống', to: '/admin/settings' },
  { icon: HelpCircle, label: 'Trung tâm hỗ trợ', to: '/admin/help' },
];
    ...
    <aside className="w-72 bg-white dark:bg-slate-950 border-r border-slate-100 dark:border-slate-900 h-full flex flex-col relative z-50 transition-colors duration-300">
      <div className="p-8 pb-6">
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-black text-brand uppercase tracking-[0.2em] mt-1.5 opacity-80">SUPER ADMIN</span>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
        <div className="px-4 pt-4 pb-2">
          <p className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-[0.2em]">Quản trị hệ thống</p>
        </div>
        ...
        <div className="px-4 pt-8 pb-2">
          <p className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-[0.2em]">Hệ thống</p>
        </div>

...
import { LayoutDashboard, BookOpen, FileText, BarChart3, Users, Settings, LogOut, Moon, Sun, GraduationCap, ChevronRight } from 'lucide-react';
...
export function StudentSidebar({ onClose }) {
  ...
  const displayRole = user?.className ? `Lớp ${user.className}` : (user?.grade ? `Khối ${user.grade}` : 'Học sinh');
    ...
    <aside className="w-72 bg-white dark:bg-slate-950 border-r border-slate-100 dark:border-slate-900 h-full flex flex-col relative z-50 transition-colors duration-300">
      <div className="p-8 pb-6">
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-black text-brand uppercase tracking-[0.2em] mt-1.5 opacity-80">STUDENT</span>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
        <div className="px-4 pt-4 pb-2">
          <p className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-[0.2em]">Khu vực học tập</p>
        </div>
        ...
        <div className="px-4 pt-8 pb-2">
          <p className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-[0.2em]">Cá nhân</p>
        </div>

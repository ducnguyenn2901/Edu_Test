  ...
  try {
    ...
    if (user && user.role === 'student') {
      ...
      if (!studentId) {
        return res
          .status(401)
          .json({ message: 'Không xác định được học sinh' });
      }
...
exports.getExamById = async (req, res) => {
  try {
    ...
    if (mode === 'attempt' && randomConfig.enabled) {
      ...
      const difficulties =
        Array.isArray(randomConfig.difficulties) &&
        randomConfig.difficulties.length > 0
      ...
      if (
        Array.isArray(randomConfig.tagFilters) &&
        randomConfig.tagFilters.length > 0
      ) {
      ...
      const desiredCount =
        typeof randomConfig.questionCount === 'number' &&
        randomConfig.questionCount > 0
...
exports.createExam = async (req, res) => {
  ...
  try {
    ...
    const questionsArray = Array.isArray(payload.questions)
      ? payload.questions
      : [];
    ...
    if (questionsArray.length > 0 && !payload.grade) {
      ...
      const grades = Array.from(
        new Set(questions.map((q) => q.grade).filter(Boolean)),
      );
    ...
    const isRandomEnabled =
      payload.randomConfig && payload.randomConfig.enabled;
...
exports.submitExam = async (req, res) => {
  try {
    ...
    const { score, correctCount, totalQuestions, timeSpent, answers } =
      req.body || {};
    ...
    if (exam.startAt && now < exam.startAt) {
      return res
        .status(400)
        .json({ message: 'Bài thi này chưa đến thời gian làm bài' });
    }

    if (exam.endAt && now > exam.endAt) {
      return res
        .status(400)
        .json({ message: 'Bài thi này đã hết thời gian làm bài' });
    }

    const maxAttempts = exam.maxAttempts;
    if (typeof maxAttempts === 'number' && maxAttempts > 0) {
      const currentAttempts = (exam.attempts || []).filter(
  ...
  try {
    ...
    const perQuestion = Array.from(questionStats.values()).map((stat) => {
      ...
      const incorrectCount =
        stat.total > 0 ? Math.max(0, stat.total - stat.correct) : 0;
      const wrongRate =
        correctRate === null ? null : Math.max(0, 1 - correctRate);
    ...
    const passRate = totalAttempts
      ? Number(((passCount / totalAttempts) * 100).toFixed(1))
      : 0;
    const failCount = totalAttempts - passCount;
    const failRate = totalAttempts
      ? Number(((failCount / totalAttempts) * 100).toFixed(1))
      : 0;

    const averageTimeSeconds = totalAttempts
    ...
    res.json({
      ...
      stats: {
        ...
        failCount,
        averageTimeSeconds,
        scoreDistribution: Object.entries(scoreBuckets).map(
  ...
  try {
    ...
    const averageScore =
      totalAttemptsCount > 0
        ? Number((totalScore / totalAttemptsCount).toFixed(1))
        : 0;
      ...
      if (myRankIndex !== -1) {
        classRank = {
          ...
          topPercentage: Math.round(
            ((myRankIndex + 1) / classmateStats.length) * 100,
          ),
        };
      }
    }

...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    const [totalQuestions, approvedQuestions, totalExams, recentQuestions, classrooms, teacher] = await Promise.all([
      ...
      Question.find({ author: teacherName }).sort({ updatedAt: -1 }).limit(5).populate('category'),
      Classroom.find({ 
        $or: [
      ...
      require('../models/User').findById(teacherId).select('students teachingGrades teachingClasses')
    ]);
    ...
    if (teacher && (teacher.teachingGrades?.length || teacher.teachingClasses?.length)) {
      ...
      if (teacher.teachingGrades?.length) scopeConditions.push({ grade: { $in: teacher.teachingGrades } });
      if (teacher.teachingClasses?.length) scopeConditions.push({ className: { $in: teacher.teachingClasses } });
      
      const scopeStudents = await require('../models/User').find({
        role: 'student',
        $or: scopeConditions
      }).select('_id');
